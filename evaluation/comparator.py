...
class IDSetComparator(Comparator):
    ...
    def load_reference(self, ref, ref_id_type):
        ...
        if self.id_type in c.SUPPORTED_DISEASE_IDS:
            ...
            self.ref_dict = eu.create_ref_dict(mapping=id_mapping, keys=set('pathway.kegg'))
...
# def compare_set_to_set(ref, ref_id_type, tar, tar_id_type, mapper: Mapper, enriched=False, threshold=0.0):
#     """
#     Compare two sets of the same type (either genes or diseases) with each other.
#     The tar set is evaluated how good it matches to the ref set.
#
...
#         if enriched:
#             reference_mapping = gm.get_enriched_attributes(gene_set=ref, id_type=ref_id_type, mapper=mapper)
#         else:
...
#     if enriched:
#         ref_dict = eu.create_ref_dict(mapping=reference_mapping, keys=c.ENRICH_KEY.keys(), enriched=True)
#     else:
#         ref_dict = eu.create_ref_dict(mapping=reference_mapping, keys=reference_mapping.columns[1:], enriched=False)
